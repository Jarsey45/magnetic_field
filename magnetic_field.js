...
const MAXIMUM_INTENSITY = 120;
const MINIMUM_INTENSITY = 8;

/* Bring out variables to reduce number of variables that need to be set.
...
function sortByStrokeWeight(filingA, filingB) {
  /* Sort by the stroke weight to minimize the
    number of times that we need to call stroke weight. */
...
function Filing(x, y, i) {
  ...
  /*
    This calculates the intensity of the magnetic field for any given filing,
    assuming that the mouse acts as the magnet's center.
    This uses the magnetic flux density equation.
  */
  ...
  /*
    Now that we have theta, we can use it to calculate the rotation of each
    filing using another equation, which we'll set to phi.
  */
  this.render = function(i) {
    ...
    radians = phi + Math.PI + tilt * frameCount / 3.5;
    ...
    /* Only set the stroke weight when necessary.
       This drastially cuts down on the calls to strokeWeight(). */
    ...
    if (this.intensity > MINIMUM_INTENSITY) {
      if (prevStrokeOpacity != strokeOpacity) {
        /* Only set the stroke opacity if the prior one is
        different than the current. This should give us a small performance increase.
        With 6000 filings, this will reduce the number of calls to stroke()
        up to half per frame. */
        prevStrokeOpacity = strokeOpacity;
        stroke(50, 200, 255, strokeOpacity);
      }
      line(x1, y1, x2, y2);
    } else {
